...
cekEndGame :-
    ...
    ( game_mode(turnamen) ->
        ...
        kartu_pemain(T1P1, KT1P1), kartu_pemain(T1P2, KT1P2),
        kartu_pemain(T2P1, KT2P1), kartu_pemain(T2P2, KT2P2),
        ...
        ( TotalTim1 < TotalTim2 -> write('Selamat, Tim 1 menjadi pemenang!') ; write('Selamat, Tim 2 menjadi pemenang!') ), nl
    ;
...
print_rincian_semua_pemain([Pemain|Sisa]) :-
    kartu_pemain(Pemain, ListKartu),
    ( ListKartu == [] -> write(Pemain), write(': kartu habis = 0 poin'), nl
    ;
        write(Pemain), write(': '),
        print_nama_kartu_deret(ListKartu), write(' = '),
        print_poin_kartu_deret(ListKartu),
        hitung_poin_tangan(ListKartu, Total),
